...
abstract class AbstractEnchantmentMachine extends AContainer {
    ...
    private final ItemSetting<List<String>> ignoredLores = new ItemSetting<>(this, "ignored-lores", Collections.singletonList("&7- &c無法被使用在" + this.getItemName() + "上"));
    ...
    protected void showEnchantmentLevelWarning(@Nonnull BlockMenu menu) {
        if (!useLevelLimit.getValue()) {
            throw new IllegalStateException("自動附/退魔機等級限制未啟用，無法顯示警告訊息。");
        }
